...
function bindDynamic() {
  ...
  document.querySelectorAll("[data-observation-stop]").forEach((button) => button.onclick = async () => { try { await request(`/api/observations/${button.dataset.observationStop}/stop`, { method: "POST", body: "{}" }); notice("Collecting the final recorded actions."); await refresh(); } catch (error) { notice(error.message, true); } });
